...
class BaseStore(object):
    ...
    def _bulk_update(self, update_tuples): raise NotImplementedError()  # return key,obj,djchanges tuples (obj is None on failure) (update_tuple=(key, obj, dchanges))    exceptions unnecessary(will get picked up in failures) 
    ...
    def _single_update(self, key, obj, dchanges): raise NotImplementedError()  # returns obj with changes  throws excption on error   # only need to implement keyless ability on keyless stores
    ...
    def bulk_update(self, tuples):
        ...
        try:
            for key,obj,dchanges in self._bulk_update(tuples):
                if obj: self._single_updated(key, obj, dchanges)
                else: self._single_update_fail(key, obj, dchanges)
        except NotImplementedError:
            for key,obj,dchanges in tuples:
                self.single_update(key, obj, dchanges)

    def single_update(self, key, obj, dchanges):
        try:
            obj = self._single_update(key, obj, dchanges)
            if not key: key = getattr(obj,self.key)
            self._single_updated(key, obj, dchanges)
        except NotImplementedError:
            raise Error("You need to implement either the _single_update or the _bulk_update")
        except StandardError as err:
            self._single_update_fail(key, obj, dchanges, err)
    ...
    def generate_update_trigger(self, obj):
        def trigger(new_key_value):
            ...
            if self.bulk_update([(new_key_value, obj, {self.key : new_key_value})]):
    ...
    def _single_updated(self, key, obj, dchanges):
        self.changes[2] += 1
        if not self.hash_.has_key(key): self.hash_[getattr(obj, self.key)] = obj
        self.log.debug('update | key=%s | dchanges=%s | obj=%s' % (key, dchanges, obj))
    ...
    def _single_update_fail(self, key, obj, dchanges, err=None):
        self.changes[3] += 1
        self.log.error('update failure | dchanges=%s | key=%s | obj=%s | err=%s' % (dchanges, key, obj, format_exc(err)))
        self._excessive_failure_check()
